      ...
      checkForMetamask().then(step1 => {
            if (step1) {
                  ...
                  tryInitContract().then(step2 => {
                        //if (step2) {  // it must be uncomment later
                        /*
                        if (false) {
                              // step 3: load my nfts;
                              GetMyNFTs();
...
//#region         step 6: Get All SOLDS

async function GetAllSolds() {
      var DoneSells = [];
      myContract.getDoneSells().then(x => {
            console.log(x);
            console.log(_hexToInt(x[0].tokenId._hex));
            ...
            for (let i = 0; i < x.length; i++) {
                  ...
                  var tokenId = _hexToInt(x[i].tokenId._hex);
                  var price = _hexToInt(x[i].price._hex);
                  var timeStamp =  x[i].timeStamp._hex;
                  // var timeStamp = _hexToInt(x[i].timeStamp._hex);
                  var wasAuction = x[i].wasAuction;

                  from = from.substring(0, 8) + '...';
                  to = to.substring(0, 8) + '...';
                  timeStamp = new Date(timeStamp*1000);
                  var newDoneSell = new DoneSell(from, to, tokenId, price, timeStamp, wasAuction, '', '', '');
                  DoneSells[i] = newDoneSell;


                  myContract._uriOf(DoneSells[i].tokenId).then(N => {
                        var data;
                        var request = new XMLHttpRequest(); // Create a request variable and assign a new XMLHttpRequest object to it.
                        request.open('GET', N); // Open a new connection, using the GET request on the URL endpoint
                        request.send();
                        request.onload = async function () {
                              data = JSON.parse(this.response);
                              DoneSells[i].name = data.name;
                              DoneSells[i].description = data.description.substring(0, 12) + '...';
                              DoneSells[i].image = data.image;
                              console.log(DoneSells[i]);
                              addDoneSell(DoneSells[i]);
                        }
                  })


            }
            console.log(DoneSells);
